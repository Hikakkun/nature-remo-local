...
def delete_ir_signal(engine : Engine, name : str):
    with Session(engine) as session:
        ...
        if ir_signal_table:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"IR signal with name '{name}' not found.")    
...
def send_ir_signal(engine : Engine, name : str, client : NatureLocalAPIClient):
    if client is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Nature Remo IP address not configured.")        
    ...
    with Session(engine) as session:
        ...
        if ir_signal_table:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error sending IR signal to Nature Remo: {e}")
        else:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"IR signal with name '{name}' not found.")    
